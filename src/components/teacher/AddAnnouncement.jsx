...
const columns = [
  {
    title: 'Pick which classes the announcement should be released.',
    dataIndex: 'name',
    render: (text) => <a>{text}</a>,
  },
];

export default function AddAnnouncement({ classList, grabAll }) {
  ...
  const postAssignment = (postData) => {
    const url = `${TEACHER_ROUTES.POST_ANNOUNCEMENT}`;
    axios.post(url, postData)
      .then(() => {
        grabAll();
        setShowForm(false);
      })
      .catch((err) => console.error('add announcement', err));
  };
  const onFinish = (fieldsValue) => {
    // Should format date value before submit.
    const values = {
      ...fieldsValue,
      expiration_date: fieldsValue.expiration_date.format('YYYY-MM-DD HH:mm:ss'),
      ...
      { showForm
        ? (
          ...
          >
            ...
            <Form.Item name="expiration_date" label="Expiration Date" {...config}>
